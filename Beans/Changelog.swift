...
enum ChangelogStore {
    ...
    static let logs: [VersionLog] = [
        VersionLog(
            ...
            features: [
                "「我的」页面新增「插件音源」入口卡片，就在本地歌单上方：点开直接是搜歌页（支持搜歌名 / 歌手 / 粘贴 BV 号），也可以看榜单、导入歌单。以前这个入口藏在音乐库顶部的平台切换里，而且装了插件才显示——插件一丢连重导的地方都找不到；现在这张卡片常驻显示，没装插件时点进去也有提示和「管理」按钮",
                "「我的页面板块排序」里同步加入「插件音源」，不喜欢这个位置可以在排序里拖走"
            ],
            fixes: [],
            imageURL: nil,
            textColorHex: nil
        ...
        VersionLog(
            ...
            features: [
                "歌单可以置顶了：音乐库里的云端歌单（网易云 / QQ / 酷狗）和本机歌单都支持长按「置顶歌单」，置顶的会排在最前面并显示一个小图钉。置顶只是本机的显示偏好，不影响歌单内容和云同步",
                "歌单里可以批量操作了：歌单页右上角新增「多选」按钮，进入后顶部显示「已选择 N 项 / 全选」，底部一条操作栏可以「下一首播放 / 收藏到歌单 / 下载 / 删除」。云端歌单的删除会同步回网易云（QQ、酷狗没有写回接口，这一项不会显示）",
                "单首歌也能直接删：歌单里的歌向左滑即可从歌单移除（云端歌单为「从歌单删除」），不用再进多选模式",
                "歌单页的「播放全部」后面带上歌曲数（如「播放全部 (610)」），排序菜单新增「歌手」"
            ],
            fixes: [
                "修复小屏（iPhone 6s / 7 / 8 / SE）上「切到歌词页就没有音量条」：屏幕高度不够时，歌词内容会把底部控制区顶出可视范围。现在底部控制区优先拿到它需要的高度，被压缩的是上方歌词区；同时按屏幕高度收紧了间距",
                "音量条在小屏上不再出现「这一行在，但滑杆看不见也拖不动」：系统音量控件的内部滑杆是按初始尺寸创建的，初始尺寸为零时可能压根不创建",
                "主页冷启动不再干等：主页数据原先只存在内存里，App 每次重新打开都必须等一轮网络才出内容（老机器上尤其明显）。现在缓存会写进本机，打开时先把上次的内容铺出来，再在后台静默刷新",
                "本地歌单里的搜索不再每敲一个字符就把整张歌单重新过滤一遍（原先它是每次重画都要算一遍的计算属性，几百首歌时输入会发涩）",
                "本地歌单滑动更跟手：播放上下文的登记原先写在每一行的构造里，等于每渲染一行就把整张歌单往注册表里写一遍；现在只在数据或搜索词变化时登记一次"
            ]
        ),
        VersionLog(
            id: "1.15.0",
            version: "1.15.0",
            title: "VIP 歌不用再干等，设置页和歌单不再一顿一顿",
            features: [],
            fixes: [
                "修复「VIP 歌一首放完就停住，要自己点播放键」：判断「这首播完了没」时，原先参照的是当时的播放器实例，而这首歌播放过程中只要发生过一次自动重试（换地址、降音质），这个参照就换成了新对象 —— 于是正常播完的通知被判为「不是这一首」而丢掉。现在改为按「当前正在播的那一份媒体」判断，重试过也一样能正常接上下一首",
                "VIP 歌的等待明显缩短：内置音源、官方接口、第三方音源的超时各收了一档（官方取址 6 秒→4 秒、降档 4 秒→3 秒、内置音源 6 秒→4 秒、第三方 7 秒→5 秒）。这些数字以前是「宁多勿少」，实际代价是每一首拿不到直链的歌都要把这几档的时间白白加在一起",
                "第三方音源加了总时间上限（8 秒）：单个音源内部是「音质档 × 密钥」挨个试的，配了三档音质、两个密钥就是六次请求首尾相接，最坏三十秒。超过总上限就放弃这一轮、直接跳到下一首，不再让你对着转圈的播放键干等",
                "「预热下一首」覆盖到了 VIP 歌：上一版只预热内置音源那一档，而 pyncmd 拿不到直链的 VIP 歌等于没预热。现在会跑完整条顺位链并把结果存好，用一首歌剩下的播放时间换下一首的零等待 —— 自动切歌那一刻直接出声",
                "进歌单后滑动不再发涩：长列表里每一行原先都带着整份歌曲列表，系统判断某一行要不要重画时得把这份列表逐首比一遍，一千首歌就是上百万次比较。现在每行只带一个短标记，需要那份列表时再取回来",
                "歌单里的搜索和排序不再每次重画都算一遍（原先每敲一个字符都会把整张歌单过滤并排序一次）",
                "设置页里「主题模式 / 平台显示 / 播放设置 / 备份」这些折叠面板展开时不再卡一下：这一下要同时把里面的控件和壁纸缩略图全部搭出来，原先还要求它在 0.22 秒的展开动画里完成 —— 现在不做展开动画，内容直接出现",
                "设置页的壁纸缩略图改为后台缩小后再显示：原先每展开一次外观面板，就要在主线程把每张壁纸整图解码一遍（一张常见壁纸解出来约 12 MB），张数一多就是几百毫秒的白屏"
            ]
        ),
        VersionLog(
            id: "1.14.0",
